...
if(isset($sub))
{
   ...
   if($ext=="jpg" || $ext=="png")
   {
     $iname=rand().".$ext";
     $pid="Kri".rand();
     if(mysqli_query($link,"INSERT INTO product (pid, cat, p_name,price,quan,disc ,warrenty,iname,descript)
        VALUES ('$pid', '".$_POST["cat"]."' , '".$_POST["p_name"]."' , '".$_POST["price"]."', '".$_POST["quan"]."', 
        '".$_POST["disc"]."','".$_POST["warrenty"]."', '$iname', '".$_POST["descript"]."')") or die("Query Problem"))
     {
...
if(mysqli_num_rows($sel)>0)
{
 ...
 <tr>
      ...
      <td><?= $arr['dat'];?></td>
      <td>
        <a href="dashboard.php?con=edit&eid=<?= $arr['id'];?>" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span></a>
